...
public final class Common {
     ...
     */
    @Getter
    private static String tellPrefix = "[VelocityControl]";

    /**
     * The log prefix applied on log() methods
     */
    @Getter
    private static String logPrefix = "[VelocityControl]";
    ...
    /**
     * Replace the & letter with the {@link LegacyComponentSerializer#SECTION_CHAR} in the message.
     * <p>
     ...
     */
    public static TextComponent colorize(final String message) {
        if (message == null || message.isEmpty())
            return Component.text("");

        TextComponent result = LegacyComponentSerializer.legacyAmpersand().deserialize(message
                .replace("{prefix}", message.startsWith(tellPrefix) ? "" : removeSurroundingSpaces(tellPrefix.trim()))
                .replace("{server}", "[Server]")
                .replace("{plugin_name}", "VelocityControl")
    ...
    public static ScheduledTask runLater(final int delayTicks, Runnable task) {
        ...
        return scheduler.buildTask(VelocityControl.getInstance(), task).delay(Duration.ofMillis(delayTicks * 50)).schedule();
    }
