...
public abstract class Operator implements org.mineacademy.bfo.model.Rule {
	...
	public abstract static class OperatorCheck<T extends Operator> {
		 ...
		 * @param message
		 */
		protected OperatorCheck(@NonNull CommandSender sender, SerializedMap variables) {
			this.variables = variables;
			this.sender = sender;

			this.isPlayer = sender instanceof ProxiedPlayer;
			this.player = isPlayer ? (ProxiedPlayer) sender : null;
		}
			...
			if (!operator.getWarnMessages().isEmpty())
				Common.runLater(1, () -> {
					for (final Entry<UUID, String> entry : operator.getWarnMessages().entrySet()) {
						...
						Common.tell(sender, replaceVariables(warnMessage, operator));
					}
				});
		...
		protected SerializedMap prepareVariables(T operator) {
			final SyncedCache cache = SyncedCache.fromName(this.sender.getName());
			final SerializedMap map = cache != null ? cache.toVariables()
					: SerializedMap.ofArray(
							...
							"player_nick", this.sender.getName(),
							"nick", this.sender.getName(),
							"player_group", "",
							"player_prefix", "",
							"player_server", this.sender instanceof ProxiedPlayer ? ((ProxiedPlayer) this.sender).getServer().getInfo().getName() : "",
							"player_afk", "false",
