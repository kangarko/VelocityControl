...
public abstract class PlayerMessage extends Operator {
	...
	public String getPrefix() {
		return Common.getOrDefault(this.prefix, Settings.Messages.PREFIX.get(this.type));
	}

	/**
	 * @see org.mineacademy.bfo.model.Rule#getUid()
	 */
	@Override
	...
	/**
	 * @see org.mineacademy.bfo.model.Rule#getFile()
	 */
	@Override
	...
	/**
	 * @see org.mineacademy.bungeecontrol.operator.Operator#onParse(java.lang.String, java.lang.String, java.lang.String[])
	 */
	@Override
	...
	public static abstract class PlayerMessageCheck<T extends PlayerMessage> extends OperatorCheck<T> {
		 ...
		 */
		protected ProxiedPlayer receiver;
		 ...
		 * @param message
		 */
		protected PlayerMessageCheck(Type type, ProxiedPlayer player, SerializedMap variables) {
		 ...
		 * @param receiver
		 */
		protected void setVariablesFor(@NonNull ProxiedPlayer receiver) {
			this.receiver = receiver;
		}

		/**
		 * @see org.mineacademy.bungeecontrol.operator.Operator.OperatorCheck#filter(org.mineacademy.bungeecontrol.operator.Operator)
		 */
		@Override
		protected void filter(T message) throws EventHandledException {
			...
			for (final ProxiedPlayer player : BungeeControl.getPlayers()) {
				...
				if (!canFilterMessage(message)) {
					Debugger.debug("operator", "\tcanFilterMessage returned false for " + player.getName());
		...
		/**
		 * @see org.mineacademy.bungeecontrol.operator.Operator.OperatorCheck#canFilter(org.mineacademy.bungeecontrol.operator.Operator)
		 */
		private boolean canFilterMessage(T operator) {
			...
			if (operator.getRequireSenderPermission() != null) {
				...
				Debugger.debug("operator", "\thas " + this.sender.getName() + " require sender permission " + replaceVariables(permission, operator) + " ? " + this.sender.hasPermission(replaceVariables(permission, operator)));
			...
			if (operator.getRequireReceiverPermission() != null) {
				...
				Debugger.debug("operator", "\thas " + this.receiver.getName() + " require receiver permission " + replaceReceiverVariables(permission, operator) + " ? " + this.receiver.hasPermission(replaceVariables(permission, operator)));
			...
			if (operator.getRequireSenderServer() != null && this.isPlayer && !this.player.getServer().getInfo().getName().equalsIgnoreCase(operator.getRequireSenderServer())) {
				Debugger.debug("operator", "\tno require sender server");

				return false;
			}

			if (operator.getRequireReceiverServer() != null && !this.receiver.getServer().getInfo().getName().equalsIgnoreCase(operator.getRequireReceiverServer())) {
			...
			if (operator.getIgnoreSenderServer() != null && this.isPlayer && this.player.getServer().getInfo().getName().equalsIgnoreCase(operator.getIgnoreSenderServer())) {
				Debugger.debug("operator", "\tignore sender server found");

				return false;
			}

			if (operator.getIgnoreReceiverServer() != null && this.receiver.getServer().getInfo().getName().equalsIgnoreCase(operator.getIgnoreReceiverServer())) {
		...
		/**
		 * @see org.mineacademy.bungeecontrol.operator.Operator.OperatorCheck#executeOperators(org.mineacademy.bungeecontrol.operator.Operator)
		 */
		@Override
		protected void executeOperators(T operator) throws EventHandledException {
			...
			if (!message.isEmpty() && !"none".equals(message)) {
				...
				if ("[JSON]".equals(prefix) || message.startsWith("[JSON]")) {
					...
					toSend = Variables.replace(message, sender);

					receiver.sendMessage(ComponentSerializer.parse(toSend));
				}
				...
				// Send as interactive format otherwise
					...
					replaced = replaceVariables(replaced.replace("{player}", getMessagePlayerForVariables().getName()), operator);

					Common.tell(receiver, replaced);
				}
		...
		/**
		 * @see org.mineacademy.chatcontrol.model.Checkable#prepareVariables(org.mineacademy.bungeecontrol.operator.Operator)
		 */
		@Override
