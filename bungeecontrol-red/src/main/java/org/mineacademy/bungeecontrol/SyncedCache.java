...
import org.mineacademy.bungeecontrol.hook.PartiesHook;
import org.mineacademy.bungeecontrol.listener.BungeeControlListener.SyncType;
import org.mineacademy.bungeecontrol.model.BungeePacket;
import org.mineacademy.bungeecontrol.model.ChannelMode;
...
public final class SyncedCache {
	...
	private void loadData(String line) {
		...
		for (final String section : sections) {
			...
			Debugger.debug("bungee-sync", "Parsing key '" + sectionName + "' with value '" + sectionValue + "'");
			...
			if ("S".equals(sectionName))
				...
				// Also sale to Parties
				if (this.uniqueId != null && BungeeControl.isPartiesFound())
					PartiesHook.setNickName(this.uniqueId, this.nick);
			}
	...
	public static void updateForOnlinePlayers() {
		synchronized (cacheMap) {
			...
			for (final ProxiedPlayer player : Remain.getOnlinePlayers()) {
			...
			for (final Iterator<Entry<String, SyncedCache>> it = cacheMap.entrySet().iterator(); it.hasNext();) {
				final Entry<String, SyncedCache> entry = it.next();
			...
			final OutgoingMessage message = new OutgoingMessage(BungeePacket.PLAYERS_CLUSTER_HEADER);
	 ...
	 * @param data
	 */
	public static void upload(SyncType syncType, SerializedMap data) {
		synchronized (cacheMap) {
	...
	public static final class Builder {
		...
		public static Set<UUID> parseUUIDList(String value) {
			return value.isEmpty() ? new HashSet<>() : new HashSet<>(Common.convert(value.split("\\|"), UUID::fromString));
		}
